...
var CustomSelect = function (_CustomDropDown) {
  _inherits(CustomSelect, _CustomDropDown);
  ...
  function CustomSelect() {
    ...
    var _this = _possibleConstructorReturn(this, (CustomSelect.__proto__ || Object.getPrototypeOf(CustomSelect)).call(this, { selector: param.selector, customClass: param.customClass, customOpenClass: param.customOpenClass, customSelectedClass: param.customSelectedClass }));

    _this.clickFakeSelectListener = function (e) {
      return _this.handleClickFakeSelect(e);
    };

    _this.selectedOption = 'Default';
    return _this;
  }
  ...
  _createClass(CustomSelect, [{
    ...
    value: function init() {
      ...
      this.inputs.forEach(function ($select) {
        $select.classList.add('hide');
        _this2.createCustomDropDown($select);
      });
    }
  }, {
    key: 'getOptions',
    value: function getOptions($select) {
      return [].concat(_toConsumableArray($select.querySelectorAll('option')));
    }
  }, {
    key: 'createFakeSelect',
    value: function createFakeSelect($select) {
      var optionContent = $select.querySelector('option').textContent;
      var $result = this.createEmptyLink(optionContent);
    ...
    value: function createCustomDropDown($select) {
    ...
    key: 'handleClickFakeSelect',
    value: function handleClickFakeSelect(e) {
      e.preventDefault();
    ...
    value: function setSelectedOption($selectedOption) {
      var _this3 = this;

      var $customDropdown = $selectedOption.parentElement.parentElement.parentElement;
      var customOptions = [].concat(_toConsumableArray($customDropdown.querySelectorAll('ul li a')));
      var $fakeSelect = $customDropdown.querySelector('a');
      var options = [].concat(_toConsumableArray($customDropdown.parentElement.querySelectorAll('option')));

      customOptions.forEach(function ($customOption, index) {
        //set the selected option in the select options
        _this3.addOrRemoveSelected(options[index].value === $selectedOption.dataset.value, options[index]);

        if ($customOption === $selectedOption) {
          //set the class for the custom option
          $customOption.classList.add(_this3.customSelectedClass);

          //set the fake select textContent
          _this3.setContentToContent($fakeSelect, $customOption);

          _this3.selectedOption = $customOption.textContent;
        } else {
          $customOption.classList.remove(_this3.customSelectedClass);
        }
      });
    }
  }, {
    key: 'addOrRemoveSelected',
    value: function addOrRemoveSelected(condition, $option) {
      if (condition) {
