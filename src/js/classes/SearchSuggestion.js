...
var _CustomDropDown2 = require('./CustomDropDown');

var _CustomDropDown3 = _interopRequireDefault(_CustomDropDown2);
...
var SearchSuggestion = function (_CustomDropDown) {
  _inherits(SearchSuggestion, _CustomDropDown);
  ...
  function SearchSuggestion() {
    ...
    //listeners
    var _this = _possibleConstructorReturn(this, (SearchSuggestion.__proto__ || Object.getPrototypeOf(SearchSuggestion)).call(this, { selector: param.selector, customClass: param.customClass, customOpenClass: param.customOpenClass, customSelectedClass: param.customSelectedClass }));

    _this.inputListener = function (e) {
      return _this.handleInput(e);
    };
    _this.ajaxResult = function (r) {
      return _this.handleAjaxResult(r);
    };
    _this.selectOption = function (e) {
      return _this.handleSelectOption(e);
    };
  ...
  _createClass(SearchSuggestion, [{
    ...
    value: function init() {
      ...
      this.inputs.forEach(function ($input) {
        $input.addEventListener('input', _this2.inputListener);
      });
    ...
    value: function handleClickOption(e) {
      e.preventDefault();
      var $option = e.currentTarget;
      var $input = $option.parentElement.parentElement.parentElement.querySelector('input');
    ...
    key: 'addToSelectedTags',
    value: function addToSelectedTags(optionData) {
      this.selectedTags.push(optionData);
    }
  }, {
    key: 'addOptionToSelectedTags',
    value: function addOptionToSelectedTags($option) {
      var optionData = this.getOptionData($option);
    ...
    key: 'checkIfTagIsSelected',
    value: function checkIfTagIsSelected(optionData) {
      return this.selectedTags.filter(function (tag) {
        return tag.type === optionData.type;
      }).length === 0;
    }
  }, {
    key: 'getOptionData',
    value: function getOptionData($option) {
    ...
    value: function createTag(data) {
      ...
      var $a = this.createEmptyLink(data.value);
    ...
    key: 'handleRemoveTag',
    value: function handleRemoveTag(e) {
      e.preventDefault();
      var $tagElem = e.currentTarget;
      var tagData = this.getDataFromTagElem($tagElem);
      this.removeTag(tagData, $tagElem);
    }
  }, {
    key: 'getDataFromTagElem',
    value: function getDataFromTagElem(tagElem) {
      return { type: tagElem.dataset.type, value: tagElem.textContent };
    }
  }, {
    key: 'removeTag',
    value: function removeTag(tagObj, $elem) {
      var _this3 = this;

      this.selectedTags.forEach(function (tag, index) {
        if (tag.type === tagObj.type && tag.value === tagObj.value) {
          _this3.selectedTags.splice(index, 1);
    ...
    key: 'handleAjaxResult',
    value: function handleAjaxResult(results) {
      var $input = document.querySelector('input[name="' + results.inputName + '"]');
      var $customSuggestion = this.getCustomSuggestion($input);
    ...
    key: 'createOptions',
    value: function createOptions(inputName, options) {
      var _this4 = this;

      var results = [];
      options.forEach(function (option) {
        var $li = document.createElement('li');
        var $a = _this4.createEmptyLink('');
        ...
        //add listener
        $a.addEventListener('click', _this4.clickOptionListener);

        //add link to list item
        _this4.addElemToElem($a, $li);
    ...
    key: 'getFilterUrl',
    value: function getFilterUrl($input) {
    ...
    key: 'getCustomSuggestion',
    value: function getCustomSuggestion($input) {
      return $input.parentElement.querySelector('.' + this.customClass);
    }
  }, {
    key: 'fillCustomDropDown',
    value: function fillCustomDropDown($input, options) {
      var $customSuggestion = this.getCustomSuggestion($input);
    ...
    key: 'wipeElement',
    value: function wipeElement($elem) {
      $elem.innerHTML = '';
    }
  }, {
    key: 'clearValue',
    value: function clearValue($elem) {
      $elem.value = '';
    }
  }, {
    key: 'createCustomDropDown',
    value: function createCustomDropDown($input) {
      var $customSuggestion = document.createElement('ul');
    ...
    value: function handleClickWindow() {
      var _this5 = this;

      this.inputs.forEach(function ($input) {
        var $customDropDown = $input.parentElement.querySelector('ul');
        if ($customDropDown) {
          _this5.wipeElement($customDropDown);
        }
      });
    }
  }]);
